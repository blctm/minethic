...
- **MP(gr)**: Cantidad en gr del metal que se está lixiviando.
- **Cantidad Total (gr)** : Masa total de la muestra utilizada en el experimento
...
MP_gr = st.sidebar.number_input("MP(gr)", value=0.0, step=0.01, format="%.2f")
...
Disolvente = st.sidebar.number_input("Disolvente", value=0.0, step=0.01, format="%.3f")
LicorLavado = st.sidebar.number_input("Licor Lavado", value=0.0, step=0.01, format="%.3f")
Acid_Concentration = st.sidebar.number_input("Concentración de ácido", step=0.01, value=0.0, format="%.f")
Volume_of_Acid = st.sidebar.number_input("Volúmen de ácido (L)", value=0.0, step=0.01, format="%.6f")
...
st.markdown("### Importancia de los Parámetros")
